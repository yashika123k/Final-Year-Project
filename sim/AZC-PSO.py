...
class Zcr:
    ...
    def _select_chs(self, node_ids, nodes, k, current_round):
        ...
        for idx, nid in enumerate(node_ids):
            ...
            score = (0.7 * energy - 0.1 * d_bs - 0.2 * d_cent) * cooldown
    ...
    def _assign_nodes(self, simulator, node_ids, ch_ids, labels, nodes, current_round):
        ch_set = set(ch_ids)
    ...
    def _handle_ch_energy(self, simulator, nodes):
        for fid in self.far_chs:
            ...
            for nid in self.near_chs:
                near_node = nodes[nid]
                if not near_node.is_alive:
                    continue
                if near_node.distance_to_base_station_m >= node.distance_to_base_station_m:
                    continue

                d = np.linalg.norm(node.position - near_node.position)
                if d < best_dist:
                    best_dist = d
                    best_near = nid
            ...
            if best_near is not None and relay_cost < direct_cost * 0.9:
                ...
                if near_node.is_alive:
                    ...
                    if clamp_and_update_liveness(near_node):
                        simulator.alive_node_count -= 1
            else:
                node.target_node_id = None
    ...
    def run_round(self, simulator: Simulator):
        ...
        self._assign_nodes(simulator, near_ids, near_chs, near_labels, nodes, simulator.current_round)
        self._assign_nodes(simulator, far_ids, far_chs, far_labels, nodes, simulator.current_round)

        self._handle_ch_energy(simulator, nodes)
